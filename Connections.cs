...
namespace GRC_Clinical_Genetics_Application
{
    class Connections : IDisposable
    {
        ...
        {
            SqlCommand nPat = new SqlCommand("SELECT count(*) FROM [GRC].[dbo].[Patients] where [Personal Health Number] = '" + PHN + "' and [Last Name] = '" + lastName + "' and [DOB] = '" + DOB + "' and Gender = '" + genderID + "' " , GRC_Connection);
            return nPat;
        }
        ...
        {
            SqlDataAdapter specialtyList = new SqlDataAdapter("SELECT [Test Type  Description] FROM [GRC].[dbo].[CBO Test Types]", GRC_Connection);
